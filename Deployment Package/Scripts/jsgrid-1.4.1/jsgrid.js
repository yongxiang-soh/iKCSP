...
(function (window, $, undefined) {
    ...
    Grid.prototype = {
        ...
        rowClick: function (args) {
            if (this.selected)
            {
                var body = $(args.event.target).closest("tbody");
                body.find("tr").removeClass("jsgrid-selected");
                var selectedRow = $(args.event.target).closest("tr");
                selectedRow.addClass("jsgrid-selected");
            }
            if (this.editing) {
        ...
        _refreshWidth: function () {
            ...
            $headerGrid.width("").css('min-width', '100%').css('width', width);
            $bodyGrid.width("").css('min-width', '100%').css('width', width);

            //this._container.width(width).css('min-width', '100%');
            this._container.css('min-width', '100%').css('max-width', '100%');
            width = $headerGrid.outerWidth();
        ...
        getSearchCondition: function () {
            ...
            $.extend(searchCondition, this.getFilter(), this._sortingParams());
