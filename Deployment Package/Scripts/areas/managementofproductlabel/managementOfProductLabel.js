...
$("body").bind("change", "#CodeLabel", function() {
    ...
    if (result === "Other") {
        $("#CodeLabel1").prop('readonly', false);
    } else {
        $("#CodeLabel1").prop('readonly', true);
    }
});
...
function ChangeDate() {
    ...
    if (mfgDate !== "") {
        ...
        result.setDate(result.getDate() + shelfLifeNumber);
...
function Print() {
    ...
    if (getExternalPrintOption() == '1' && getInternalPrintOption() == '1') {
        return;
    }
...
isStsPartNoSolid = function () {
    ...
    if (externalLabelOption == '6' || externalLabelOption == '7' || externalLabelOption == '8') {
