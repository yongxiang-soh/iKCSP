...
{
    [MvcAuthorize("TCFC203F")]
    public class ProductCerfiticateConsumedListController : KCSG.Web.Controllers.BaseController
    {
        #region Constructors
        ...
        /// </summary>
        /// <param name="ProductCerfiticateConsumedListDomain"></param>
        /// <param name="exportReportDomain"></param>
        public ProductCerfiticateConsumedListController(IManagementReportDomain ProductCerfiticateConsumedListDomain,
            IExportReportDomain exportReportDomain)
        {
            _ProductCerfiticateConsumedListDomain = ProductCerfiticateConsumedListDomain;
        ...
        /// </summary>
        private readonly IManagementReportDomain _ProductCerfiticateConsumedListDomain;
        ...
        public async Task<ActionResult> RetrieveTableRecords(string yearmonth, int page)
        {
            try
            {
                var loadKneadingCommandResult = await _ProductCerfiticateConsumedListDomain.LoadConsumerCerfiticates(yearmonth,page);
        ...
        [HttpPost]
        public async Task<ActionResult> CalculateTableRecords(string yearmonth,string procode)
        {
            try
            {
                var loadKneadingCommandResult = await _ProductCerfiticateConsumedListDomain.RecalculateCerfiticate(yearmonth,procode);
                return Json(loadKneadingCommandResult.Data);
            }
            catch (Exception exception)
            {
        ...
        public async Task<ActionResult> UpdateManagementReport(UpdateManagementReportViewModel parameters)
        {
            ...
            // Paramenter valid
            var updateConsumerMaterial = _ProductCerfiticateConsumedListDomain.UpdateConsumerCerfiticate(parameters.YearMonth, parameters.Recieved, parameters.Remain, parameters.Used, parameters.ProductCode);
        ...
        public async Task<ActionResult> Checkitemexist(string yearmonth)
        {
                ...
                var exist = _ProductCerfiticateConsumedListDomain.CheckConsumerCerfiticates(yearmonth);
                return Json(exist);
