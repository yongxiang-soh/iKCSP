...
{
    [MvcAuthorize("TCFC111P")]
    public class MaterialRetrievalRecordController : KCSG.Web.Controllers.BaseController
    {
        ...
        public MaterialRetrievalRecordController(IManagementReportDomain ManagementReportDomain,
        ...
        public ActionResult Index()
        {
            ...
            return View(model);
                ...
                [HttpPost]
        public async Task<ActionResult> ExportMaterialRetrievalRecord(string from, string to)
        {
            try
            {                
                ...
                // check status
                    ...
                    var result =
                    await _ManagementReportDomain.SearchMaterialRetrievalRecord(from,to);
                // Export the template.
                if (result.FindPrintManagementReportItem.Count > 0)
                {
                    // Find the template file.
                    exportMaterialNameAbsolutePath =
                        Server.MapPath(string.Format("~/{0}", Constants.ExportTemplate.MaterialRetrievalRecord));
                ...
                {// Find the template file.
                    exportMaterialNameAbsolutePath =
                        Server.MapPath(string.Format("~/{0}", Constants.ExportTemplate.MaterialRetrievalRecordEmpty));
